...
# # from notebookllama.mrkdwn_analysis import MarkdownAnalyzer
...
if (
    os.getenv("LLAMACLOUD_API_KEY")
    and os.getenv("EXTRACT_AGENT_ID")
    and os.getenv("LLAMACLOUD_PIPELINE_ID")
    ...
    if os.path.exists(path):
        for image_file in os.listdir(path):
            ...
            if os.path.isfile(image_path) and "_at_" not in image_path:
                ...
                with open(new_path, "wb") as img_tw:
                    img_tw.write(bts)
                renamed.append(new_path)
                os.remove(image_path)
    return renamed
...
async def parse_file(
    ...
    if with_tables and text:
        analyzer = MarkdownTextAnalyzer(text)
        table_results = analyzer.identify_tables()
        if "Table" not in table_results:
            warnings.warn("\u26a0\ufe0f No 'Table' key found in identify_tables() output.")
        md_tables = table_results.get("Table", [])
        tables = []
        for md_table in md_tables:
            table = md_table_to_pd_dataframe(md_table=md_table)
            if table is not None:
...
async def process_file(filename: str) -> Union[Tuple[str, None], Tuple[None, None], Tuple[str, str]]:
    with open(filename, "rb") as f:
        file = await CLIENT.files.upload_file(upload_file=f)
    files = [{"file_id": file.id}]
    await CLIENT.pipelines.add_files_to_pipeline_api(
...
def verify_claim(claim: str, sources: str) -> Tuple[bool, Optional[List[str]]]:
    response = LLM_VERIFIER.chat(
        [
            ChatMessage(
                role="user",
