...
#     engine_url=f"postgresql+psycopg2://{os.getenv('pgql_user')}:{os.getenv('pgql_psw')}@3.141.5.201:{os.getenv('pgql_port')}/{os.getenv('pgql_db')}",
...
print("🔧 Connecting to:", engine_url)
...
st.set_page_config(
    ...
    menu_items={
        "Get Help": "https://github.com/run-llama/notebooklm-clone/discussions/categories/general",
        "Report a bug": "https://github.com/run-llama/notebooklm-clone/issues/",
        "About": "An OSS alternative to NotebookLM that runs with the power of a fluffy Llama!",
    },
)
...
file_input = st.file_uploader("Upload your source PDF file!", accept_multiple_files=False)
