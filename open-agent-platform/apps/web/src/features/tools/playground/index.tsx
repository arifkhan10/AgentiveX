...
export default function ToolsPlaygroundInterface() {
  ...
  const [authRequiredMessage, setAuthRequiredMessage] = useState<ReactNode>(null);
  ...
  useEffect(() => {
    ...
    const toolNameParam = new URLSearchParams(window.location.search).get("tool");
    if (!toolNameParam) {
      router.replace("/tools");
      return;
    }

    const tool = tools.find((tool) => tool.name === toolNameParam);
  ...
  }, [tools, loading]); // ✅ Fixed infinite loop by removing searchParams
  ...
  const handleSubmit = async () => {
    ...
    try {
      const toolRes = await callTool({
        ...
        args: inputValues,
        source: selectedTool.source || "arcade",
      });
      setResponse(toolRes);
      setInputValues({});
    } catch (e: any) {
  ...
  if (!selectedTool) {
    return <div className="p-6 text-gray-600">Loading tools...</div>;
  }
  ...
  return (
    <div className="flex h-full w-full flex-col p-8">
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-grow border-0">
